...
The initial registry migration backfills active declarations only. Pre-PIE-132 Trash content can contain indexed property-shaped examples and copied Work IDs that never established symbolic identity; importing those would either create false addresses or block startup. Restoring a legacy Trash subtree transactionally registers every newly active declaration, while blocks deleted after PIE-132 retain their existing registry rows throughout Trash.
